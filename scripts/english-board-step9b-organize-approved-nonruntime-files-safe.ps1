...
$StatusBefore = @(Invoke-GitNative @("status", "--porcelain") | Where-Object { -not [string]::IsNullOrWhiteSpace($_) })
...
foreach ($item in $Items) {
    ...
    $sourceHash = Get-FileSha256 $item.Source
...
foreach ($item in $Items) {
    ...
    $destHash = Get-FileSha256 $item.Destination
...
$Lines.Add("STEP`t9B_ORGANIZE_APPROVED_NONRUNTIME_FILES_SAFE")
...
Write-Host "STEP=9B_ORGANIZE_APPROVED_NONRUNTIME_FILES_SAFE"
