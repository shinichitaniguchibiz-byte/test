...
$ReportPath = Join-Path $ReportDir ("english_board_approved_nonruntime_organization_{0}.txt" -f $Timestamp)
...
function Git([string[]]$Args) {
    ...
    $code = $LASTEXITCODE
    if ($code -ne 0) {
        throw "git failed ($code): git $($Args -join ' ')`n$($output -join "`n")"
    }
    return @($output)
}

function GitText([string[]]$Args) {
    return ((Git $Args) -join "`n").Trim()
}
...
$Branch = GitText @("symbolic-ref", "--short", "HEAD")
if ($Branch -ne "main") { Fail "Unexpected branch: $Branch" }

$StatusBefore = @(Git @("status", "--porcelain"))
if ($StatusBefore.Count -ne 0 -and -not ($StatusBefore.Count -eq 1 -and [string]::IsNullOrWhiteSpace($StatusBefore[0]))) {
    Fail "Worktree must be clean before organization."
}
...
$IgnoredAudioBefore = @(Git @("ls-files", "--others", "--ignored", "--exclude-standard", "--", "audio/")).Count
$IgnoredImageBefore = @(Git @("ls-files", "--others", "--ignored", "--exclude-standard", "--", "image/")).Count
...
Write-Host "PHASE=VERIFY_SOURCES"
...
foreach ($item in $Items) {
    ...
    $destHash = Hash $item.Destination
    if ($destHash -ne $item.Sha256) {
        Fail "Destination hash mismatch after copy: $($item.Destination)"
    }
}

$ExpectedUntracked = @($Items.Relative | Sort-Object)
$ActualUntracked = @(Git @("ls-files", "--others", "--exclude-standard") | Where-Object { -not [string]::IsNullOrWhiteSpace($_) } | Sort-Object)
...
foreach ($item in $Items) {
    Git @("add", "--", $item.Relative) | Out-Null
}
...
$TrackedAfter = @(Git @("ls-files")).Count
...
$StatusAfterCommit = @(Git @("status", "--porcelain") | Where-Object { -not [string]::IsNullOrWhiteSpace($_) })
if ($StatusAfterCommit.Count -ne 0) {
    Fail "Worktree is not clean after commit."
}

$IgnoredAudioAfter = @(Git @("ls-files", "--others", "--ignored", "--exclude-standard", "--", "audio/")).Count
$IgnoredImageAfter = @(Git @("ls-files", "--others", "--ignored", "--exclude-standard", "--", "image/")).Count
...
$RemoteBeforePush = (& git -C $Root ls-remote origin refs/heads/main 2>$null | Out-String).Trim()
...
$RemoteAfterPush = (& git -C $Root ls-remote origin refs/heads/main 2>$null | Out-String).Trim()
...
foreach ($item in $Items) {
    if ((Hash $item.Destination) -ne $item.Sha256) {
...
$FinalStatus = @(Git @("status", "--porcelain") | Where-Object { -not [string]::IsNullOrWhiteSpace($_) })
...
$Lines.Add("STEP`t9_ORGANIZE_APPROVED_NONRUNTIME_FILES")
...
Write-Host "STEP=9_ORGANIZE_APPROVED_NONRUNTIME_FILES"
