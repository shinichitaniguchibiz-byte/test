...
if ($null -eq $GhCommand) {
    ...
    Write-Host "STEP=7B_CREATE_PRIVATE_REMOTE_AND_PUSH_SAFE"
...
$Auth = Invoke-NativeCapture -FilePath $GhPath -CommandLine "auth status --hostname github.com"
...
$View = Invoke-NativeCapture -FilePath $GhPath -CommandLine "repo view $Repo --json nameWithOwner,visibility"
if ($View.ExitCode -ne 0) {
    $Create = Invoke-NativeCapture -FilePath $GhPath -CommandLine "repo create $Repo --private"
...
try {
    $RepoInfo = $View.StdOut | ConvertFrom-Json
}
catch {
    throw "Cannot parse GitHub repository metadata JSON: $($View.StdOut)"
}

if ($RepoInfo.nameWithOwner -ne $Repo) {
    throw "Unexpected repository identity: $($RepoInfo.nameWithOwner)"
}
if (($RepoInfo.visibility.ToString().ToUpperInvariant()) -ne "PRIVATE") {
    throw "Unexpected repository visibility: $($RepoInfo.visibility)"
}
...
$LsBefore = Invoke-NativeCapture -FilePath "git.exe" -CommandLine "ls-remote origin refs/heads/main"
...
if ([string]::IsNullOrWhiteSpace($LsBefore.StdOut)) {
    $Push = Invoke-NativeCapture -FilePath "git.exe" -CommandLine "push -u origin main"
...
$LsAfter = Invoke-NativeCapture -FilePath "git.exe" -CommandLine "ls-remote origin refs/heads/main"
...
$Lines.Add("STEP`t7B_CREATE_PRIVATE_REMOTE_AND_PUSH_SAFE")
