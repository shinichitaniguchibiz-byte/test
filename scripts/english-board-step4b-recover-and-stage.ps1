...
$ReportPath = Join-Path $ReportDir ("english_board_stage_recovery_{0}.txt" -f $Timestamp)
...
# No prior staging is expected. Refuse to alter an unexpected index.
$PreStaged = @(git diff --cached --name-only)
if ($PreStaged.Count -ne 0) {
    throw "Unexpected staged files already exist: $($PreStaged.Count)"
}

# The five hold files are ASCII-named and can be verified safely under Windows PowerShell 5.1.
$Moves = @(
...
# Validate all ASCII-named Git candidates explicitly.
$ExpectedAscii = @(
    ".gitignore",
    "CEFR.txt",
...
$MissingAscii = New-Object System.Collections.Generic.List[string]
foreach ($Path in $ExpectedAscii) {
    if ($Actual -notcontains $Path) {
        $MissingAscii.Add($Path)
    }
}
...
# Exactly three additional root-level TXT files are expected. Their names may be non-ASCII,
# so do not embed those names in this script.
$NonAsciiRootTxt = @($Actual | Where-Object {
    ($_ -notin $ExpectedAscii) -and ($_ -notmatch '/') -and ($_ -match '\.txt$')
})
if ($NonAsciiRootTxt.Count -ne 3) {
    throw "Unexpected non-ASCII/root TXT candidate count: expected=3 actual=$($NonAsciiRootTxt.Count)"
}

$Unexpected = @($Actual | Where-Object {
    ($_ -notin $ExpectedAscii) -and ($_ -notin $NonAsciiRootTxt)
})
if ($Unexpected.Count -ne 0) {
    throw "Unexpected Git candidates remain: $($Unexpected -join '; ')"
}
...
# Stage all validated, non-ignored files. Ignored media remain outside the index.
git add -A
...
$Lines.Add("STEP`t4B_RECOVER_AND_STAGE")
...
$Lines.Add("NON_ASCII_ROOT_TXT_COUNT`t$($NonAsciiRootTxt.Count)")
foreach ($Path in $NonAsciiRootTxt) {
    $FullPath = Join-Path $Root $Path
    $Hash = (Get-FileHash -LiteralPath $FullPath -Algorithm SHA256).Hash
    $Size = (Get-Item -LiteralPath $FullPath).Length
    $Lines.Add("ROOT_TXT`t$Path`t$Size`t$Hash")
}
...
Write-Host "STEP=4B_RECOVER_AND_STAGE"
Write-Host "ROOT=$Root"
Write-Host "HOLD_FILE_COUNT=$($Moves.Count)"
Write-Host "STAGED_COUNT=$($Staged.Count)"
Write-Host "NON_ASCII_ROOT_TXT_COUNT=$($NonAsciiRootTxt.Count)"
